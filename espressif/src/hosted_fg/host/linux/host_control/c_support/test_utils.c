...
int test_set_serial_if_file(char* if_path)
{
	FILE *fp;
	...
	if (fp != NULL) {
		memcpy(serial_file, if_path, sizeof(serial_file));
		fclose(fp);
	} else {
		printf("file is not exit!, pleae check!\n");
		return FAILURE;
	}
	
	printf("Set Serial file:%s\n", serial_file);
