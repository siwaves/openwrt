...
static void inline usage(char *argv[])
{
	printf("sudo %s option -D device\n[\n %s\t\t||\n %s\t\t||\n %s\t\t||\n %s\t\t||\n %s\t\t||\n %s\t\t\t||\n %s\t\t\t||\n %s\t\t\t||\n %s\t\t\t||\n %s\t\t\t||\n %s\t\t||\n %s\t\t||\n %s\t\t\t||\n %s\t\t||\n %s\t||\n %s\t\t\t||\n %s\t||\n %s\t||\n %s\t\t||\n %s\t\t||\n %s\t\t||\n %s <ESP 'network_adapter.bin' path>\n]\n",
		...
		SOFTAP_STOP, SET_WIFI_POWERSAVE_MODE, GET_WIFI_POWERSAVE_MODE, SET_WIFI_MAX_TX_POWER, GET_WIFI_CURR_TX_POWER,SET_SERIAL_IF_FILE,
		OTA);
	printf("\n\nFor example, \nsudo %s %s\n",
...
static int parse_cli_cmd(char *in_cmd, char *args[])
{
	...
	/* Set serial interface file*/
	else if (0 == strncasecmp(SET_SERIAL_IF_FILE, in_cmd, sizeof(SET_SERIAL_IF_FILE))) {
		printf("serial file: %s\n",args[0]);
		test_set_serial_if_file(args[0]);
	}
